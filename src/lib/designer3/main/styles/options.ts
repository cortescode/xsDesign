...
export function loadConditionalTypeSectors(editor: Editor) {


    editor.on('component:selected', (selectedComponent: Component) => {
        loadConditionalSectors(editor, selectedComponent)
    });

}

function loadConditionalSectors(editor: Editor, selectedComponent: Component) {
    ...
    for (let option of options) {
        if (option.name === name) {
            for (let section of option.sections) {
                if (!RestringedSections.includes(section.name)) {
                    const sectorExists = StyleManager.getSector(section.name);
                    if (!sectorExists)
                        StyleManager.addSector(section.name, section)
                }
            }
