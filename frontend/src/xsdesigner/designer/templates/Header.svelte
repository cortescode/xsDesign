...
<header id="header">
    ...
    <nav class="links-list">
        {#each links as { name }, index}
            ...
            {/if} -->
        {/each}
        <button class="add-link-button" on:click={addLink}>+ add link</button>
...
<style>
    header {
        display: grid;
        grid-template-columns: 80fr 4fr 80fr;
        align-items: center;
    ...
    .logo-header {
        ...
        text-align: left;
        background: linear-gradient(to right, #7fceff, #0d2aac);
    ...
    .links-list:hover .add-link-button {
        display: block;
    }
